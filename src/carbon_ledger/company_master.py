...
def reconcile_facilities(
    *,
    ...
    def _ensure(name: str, source: str) -> FacilityMasterRecord:
        key = normalize_site_name(name)
    ...
    for hint in official:
        record = _ensure(hint.display_name, SOURCE_OFFICIAL_FACTORY)
    ...
    for current in existing:
        key = normalize_site_name(current.display_name)
        if key in by_key:
            live = by_key[key]
