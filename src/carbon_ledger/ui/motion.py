...
def mark_result_reveal_pending(session_state: Any, result: PipelineRunResult) -> None:
    ...
    session_state[STATE_RESULT_REVEAL_PENDING] = animation_run_token(
...
def _should_defer_countup_runtime() -> bool:
    """True on the first painted results frame, before the count-up JS boots."""
    try:
        play = _ss_get(st.session_state, STATE_HERO_EMISSIONS_PLAY)
        ready = bool(_ss_get(st.session_state, STATE_COUNTUP_RUNTIME_READY))
    except Exception:  # noqa: BLE001 - AppTest session proxies vary
        return False
    return bool(play) and not ready
...
def should_open_analysis_progress_dialog(
    ...
    """Open the blocking modal for a new run, in-flight work, or failure."""
    if run_clicked:
        return True
...
def _progress_checklist_markdown(
    *,
    ...
    lines = [
        f"### {t('analysis.running_title', lang)}",
        f"**{int(round(max(0.0, min(100.0, percent))))}%**",
        "",
    ]
...
def execute_analysis_with_progress(
    ...
    *,
    ...
    st.markdown(
        '<div data-cel-analysis-modal="1" role="status" aria-live="polite">',
    ...
    st.success(t("analysis.complete_banner", lang))
    session_state[STATE_ANALYSIS_RUNNING] = False
    session_state[STATE_ANALYSIS_PHASE] = ANALYSIS_PHASE_COMPLETE
...
def launch_analysis_progress_dialog(
    ...
    *,
    ...
    """Open a blocking progress dialog, run the real pipeline, then close."""
    phase = analysis_phase(session_state)
    failed = phase == ANALYSIS_PHASE_FAILED
    complete = phase == ANALYSIS_PHASE_COMPLETE
    if failed:
        title = t("error.analysis_incomplete", lang)
    elif complete:
        title = t("analysis.complete_banner", lang)
    else:
        title = t("analysis.running_title", lang)

    @st.dialog(title, width="medium", dismissible=failed)
    def _dialog() -> None:
        st.markdown(
            '<div class="cel-analysis-modal" data-cel-analysis-modal="1" '
            'role="status" aria-live="polite"></div>',
            unsafe_allow_html=True,
        )
        if analysis_phase(session_state) == ANALYSIS_PHASE_FAILED:
            _render_analysis_failure(session_state, lang)
            return
        if analysis_phase(session_state) == ANALYSIS_PHASE_COMPLETE:
            st.success(t("analysis.complete_banner", lang))
            session_state[STATE_ANALYSIS_PHASE] = ANALYSIS_PHASE_REVEAL
            session_state[STATE_NAVIGATE_TO_RESULTS] = True
            st.rerun()
            return
        try:
