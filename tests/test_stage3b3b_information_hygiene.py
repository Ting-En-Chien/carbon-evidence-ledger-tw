...
def test_customer_reporting_hides_run_id_and_registry_by_default(
    ...
    assert "碳排摘要" in text or "Emissions summary" in text
    assert t("aud.advanced_audit", ZH) in text or "稽核追溯" in text
    assert "專業覆核" in text or "Technical review" in text
...
def test_admin_mode_retains_technical_reporting(monkeypatch, tmp_path) -> None:
