...
from helpers import (  # noqa: E402
    ...
    INTAKE_APPLY,
    choose_selectbox,
    click_finish_intake_confirmation,
...
def _confirm_count(text: str) -> int:
    match = re.search(r"完成剩餘 (\d+) 項", text)
    assert match, f"confirmation count missing in: {text[:500]}"
    return int(match.group(1))


def _apply_button(page):
    return page.get_by_role("button", name=re.compile(INTAKE_APPLY))
...
def test_journey_42fc1_a_zero_exception_desktop(page) -> None:
    ...
    assert page.get_by_role("button", name="完成確認並檢查資料").count() >= 1
...
def test_journey_42fc1_b_medium_exception(page) -> None:
    ...
    click_finish_intake_confirmation(page)
    wait_streamlit_idle(page, timeout=40)
    assert "可納入計算" in visible_text(page)
...
def test_journey_42fc1_d_mixed_queue_to_zero(page) -> None:
    ...
    assert re.search(r"需要確認\s*[1-9]\d*[／/]", zero) is None
...
def test_journey_42fc1_e_unresolved_rows_held_out(page) -> None:
    ...
    apply_btn.first.click(force=True)
    wait_streamlit_idle(page, timeout=40)
    page.get_by_text("「雜項能源」是哪一種活動？").first.wait_for(
    ...
    assert "可納入計算" in body
    assert "需要確認" in body
    assert "資料筆數" in body
    tab = page.get_by_role("tab", name="無法納入")
    tab.first.click()
    wait_streamlit_idle(page)
...
def test_journey_42fc1_unknown_context_held_summary(page) -> None:
    ...
    unknown = page.get_by_text("我現在無法確認，暫不納入計算", exact=True)
    ...
    assert "可納入計算" in result
    assert "需要確認" in result
