...
def e2e_company_workspace_dir(tmp_path_factory) -> Path:
    return tmp_path_factory.mktemp("company-workspaces")


@pytest.fixture(scope="session")
def e2e_base_url(e2e_company_workspace_dir: Path) -> str:
    pytest.importorskip("playwright")
    port = _free_port()
    env = os.environ.copy()
    env.pop("CEL_APP_MODE", None)  # force CUSTOMER default
    env["CEL_ZERO_ENTRY_STUB"] = "1"
    env["CEL_COMPANY_WORKSPACE_DIR"] = str(e2e_company_workspace_dir)
    ...
    log_path = ARTIFACTS / "streamlit_e2e.log"
...
@pytest.fixture(scope="session")
def e2e_browser(e2e_base_url: str):
    ...
    with sync_playwright() as playwright:
        ...
        yield browser, e2e_base_url
        browser.close()


@pytest.fixture
def page(e2e_browser, request):
    browser, base_url = e2e_browser
