...
def test_audit_zip_under_audit_package_not_management_summary() -> None:
    page = (REPO_ROOT / "app_pages/audit_export.py").read_text(encoding="utf-8")
    mgmt = page.index("aud.group.management")
    pkg = page.index("aud.group.audit_pkg")
    zip_key = page.index("aud_zip_dl")
    assert mgmt < pkg < zip_key
    management_block = page[mgmt:pkg]
    assert "aud_zip_dl" not in management_block
    assert "aud.group.management_unavailable" in management_block
    at = _run("app_pages/audit_export.py", demo=True)
    text = _surface(at)
    assert "管理摘要" in text
    assert "尚未建立" in text
    ...
    assert not any("管理摘要" in label for label in labels)


def test_run_id_hidden_by_default() -> None:
    page = (REPO_ROOT / "app_pages/audit_export.py").read_text(encoding="utf-8")
...
def test_ifrs_readiness_labeled_climate_metrics_not_compliance() -> None:
