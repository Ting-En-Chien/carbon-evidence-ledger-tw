...
def test_apptest_step5_has_no_raw_html_leak() -> None:
    ...
    from carbon_ledger.ui.tutorial import dismiss_onboarding

    at = AppTest.from_file(str(REPO_ROOT / "streamlit_app.py"), default_timeout=120)
    at.run()
    initialize_ui_state(at.session_state)
    # This test inspects applicability results, not first-run onboarding.
    dismiss_onboarding(at.session_state)
    ...
    setup = [
        ...
        if str(button.label) in {"開始公司設定", "繼續公司設定"}
    ]
    assert setup, [str(button.label) for button in at.button]
    setup[0].click()
    at.run()
