...
from carbon_ledger.ui.tutorial import get_onboarding_copy
...
def test_no_admin_language_in_customer_copy() -> None:
    ...
    assert hits == []
    onboarding = get_onboarding_copy(ZH)
    blob = " ".join(
        [
            onboarding["welcome_title"],
            onboarding["welcome_body"],
            *[step["title"] for step in onboarding["steps"]],
            *[step["body"] for step in onboarding["steps"]],
        ]
    )
...
def test_first_run_onboarding_is_concise() -> None:
    copy = get_onboarding_copy(ZH)
    assert copy["welcome_title"] == "完成第一筆碳排計算"
    assert "Excel" in copy["welcome_body"]
    ...
    # One title plus one short sentence per coachmark — nothing longer.
    for step in copy["steps"]:
        assert len(step["body"]) <= 40
        assert step["body"].count("。") <= 1
