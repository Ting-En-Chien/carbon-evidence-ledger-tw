...
def test_progress_modal_markup_and_real_stages() -> None:
    ...
    assert "render_analysis_transition_view" in source
    assert "@st.dialog" not in source
    assert "data-cel-analysis-view" in source
...
def test_dashboard_hides_results_while_analyzing() -> None:
    ...
    assert "app_pages/analysis_progress.py" in app
    assert "st.switch_page(\"app_pages/analysis_progress.py\")" in app
    assert "navigation.run()" in app
    click_at = app.find('st.switch_page("app_pages/analysis_progress.py")')
    closing_at = app.find(
        "if analysis_phase(st.session_state) == ANALYSIS_PHASE_CLOSING:"
    )
    dash_at = app.find('st.switch_page("app_pages/dashboard.py")')
    nav_at = app.find("navigation.run()")
    assert 0 <= click_at < closing_at < dash_at < nav_at
    assert "render_analysis_transition_view" not in app
    motion = (REPO_ROOT / "src/carbon_ledger/ui/motion.py").read_text(encoding="utf-8")
    assert "ANALYSIS_PHASE_CLOSING" in motion
    ...
    assert "dashboard.py" not in page
